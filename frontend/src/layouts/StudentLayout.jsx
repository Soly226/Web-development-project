...
const navItems = [
  ...
  { icon: 'menu_book', label: 'Courses', path: '/student/courses' },
  { icon: 'grade', label: 'Grades', path: '/student/grades' },
  { icon: 'calendar_month', label: 'Calendar', path: '/student/calendar' },
  ...
  return (
    <div className="relative flex min-h-screen w-full flex-col bg-background-light dark:bg-background-dark overflow-x-hidden pb-28 font-display transition-colors duration-500">
      ...
      <div className="fixed top-0 left-0 w-full h-full pointer-events-none overflow-hidden -z-10">
        <div className="absolute top-[-15%] left-[-5%] w-[45%] h-[45%] bg-primary/10 blur-[140px] rounded-full" />
        <div className="absolute top-[40%] right-[-10%] w-[35%] h-[35%] bg-accent/10 blur-[120px] rounded-full" />
        <div className="absolute bottom-[5%] left-[20%] w-[30%] h-[30%] bg-cyan-500/5 blur-[100px] rounded-full" />
      </div>
      ...
      <header className="flex items-center justify-between bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl px-5 py-3.5 border-b border-white/20 dark:border-white/10 sticky top-0 z-40 shadow-sm">
        ...
        <div className="flex items-center gap-2">
          <Link to="/messages" className="relative p-2 text-slate-500 dark:text-slate-400 hover:text-primary transition-colors rounded-xl hover:bg-primary/10">
            <span className="material-symbols-outlined text-xl">mail</span>
          </Link>
          <Link to="/notifications" className="relative p-2 text-slate-500 dark:text-slate-400 hover:text-primary transition-colors rounded-xl hover:bg-primary/10">
            <span className="material-symbols-outlined text-xl">notifications</span>
            <span className="absolute top-2 right-2 w-2 h-2 bg-rose-500 rounded-full border-2 border-white dark:border-slate-900" />
          </Link>
          <Link
            to="/student/profile"
            className="w-9 h-9 rounded-xl bg-gradient-to-br from-primary to-accent flex items-center justify-center text-white text-sm font-black shadow-md hover:brightness-110 transition-all border-2 border-primary/20 hover:border-primary"
          >
              ...
              return (
                <Link
                  ...
                  className={twMerge(
                    ...
                    active ? 'text-primary scale-105' : 'text-slate-400 dark:text-slate-500 hover:text-primary'
                  )}
                >
                  <div className={twMerge('p-1.5 rounded-xl transition-all', active && 'bg-primary/10')}>
                    <span
                  ...
                  {active && <span className="absolute -bottom-1 w-1 h-1 bg-primary rounded-full shadow-[0_0_6px_rgba(99,102,241,1)]" />}
                </Link>
              );
            })}
          </div>
